...
def lenet(only_logits=False):
  ...
  net = tf.layers.conv2d(images, 10, [5, 5], activation=tf.nn.relu, kernel_regularizer=tf.nn.l2_loss, name='conv1')
  net = tf.layers.max_pooling2d(net, [2, 2], 2, name='pool1')
  net = tf.layers.conv2d(net, 20, [5, 5], activation=tf.nn.relu, kernel_regularizer=tf.nn.l2_loss, name='conv2')
  net = tf.layers.max_pooling2d(net, [2, 2], 2, name='pool2')
  net = tf.layers.flatten(net)

  net = tf.layers.dense(net, 50, activation=tf.nn.relu, kernel_regularizer=tf.nn.l2_loss, name='fc3')
  net = tf.layers.dropout(net, 1 - dropout_keep_prob, training=is_training_ph, name='dropout3')
  logits = tf.layers.dense(net, num_classes, activation=None, kernel_regularizer=tf.nn.l2_loss, name='fc4')
